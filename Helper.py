...
def AddTypes(in_lyr):
   """
   Add site type(s) to ConSites, based on SITE_TYPE field 
...
def MatchTypes(in_lyr, deleteRows=True):
   """
   Used to delete rows from a EO/site spatial join output table where sites and EOs do not have the same site type.
   :param in_lyr: Input layer (result of spatial join)
   ...
   todel = [str(a[0]) for a in arcpy.da.SearchCursor(lyr, [oid, 'SITE_TYPE_EO', 'SITE_TYPE_CS']) if a[2] not in a[1]]
   if len(todel) > 0:
      where_clause = oid + " IN (" + ",".join(todel) + ")"
      arcpy.SelectLayerByAttribute_management(lyr, "NEW_SELECTION", where_clause)
      # printMsg(str(countSelectedFeatures(lyr)) + " rows do not match site types.")
      if deleteRows:
