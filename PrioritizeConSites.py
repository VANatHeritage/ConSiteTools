...
def updatePortfolio(in_procEOs, in_ConSites, in_sumTab, slopFactor ="15 METERS", slotDict=None):
   ...
   fields = ["Irreplaceable", "Critical", "Vital", "High_Priority", "General"]
   for fld in fields:
      try:
...
def BuildPortfolio(in_sortedEOs, out_sortedEOs, in_sumTab, out_sumTab, in_ConSites, out_ConSites, out_Excel, in_consLands_flat, build = "NEW", slopFactor = "15 METERS"):
   ...
   if build == 'NEW':
      ...
      code_block = '''def fn(myMin):
         if myMin == 1:
            ...
            tier = "Unassigned"  # Note: this is not a final tier; updated later in this function.
         else:
            tier = "General"
         return tier
      '''
   ...
   printMsg("Updating " + str(countFeatures("lyr_CS")) + " unassigned ConSites not in portfolio to General.")
